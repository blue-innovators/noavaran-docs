...
سپس به این شناسه یوزر دسترسی vloume های سرویس mssql را می دهیم. به این صورت :
...
بجای VOLUME_DIRETORY بایستی مقادیر voulme های سرویس mssql را جایگذاری نمایید که دو مقدار زیر می باشد :
...
حال بایستی یک سری تنظیمات را روی سرور minio انجام دهیم و عکس‌های اولیه را ایجاد نماییم. برای شروع آدرس زیر را در مرورگز خود باز نمایید: 
...
اکنون Gateway را بایستی راه اندازی کنیم تا بتوانیم از آدرس های مورد نظرمان به سرویس ها دسترسی داشته باشیم. بای اینکار به مانند مرحله قبل دستور زیر را اجرا میکنیم : 
...
برای این منظور لازم است که ابتدا توکن جهت کار کردن با سیستم common دریافت نمایید. به شکل زیر می توانید درخواست توکن بدهید: 
...
راه اندازی برای دفعات بعد نیاز به مراحل مشابه راه اندازی اول ندارد. کافیست تنها دستورات اجرای داکر کامپوزها دلخواه خود را به ترتیب اجرا نمایید. نکته مهم در این قسمت این است که حتماٌ در ابتدا Infrastructure را اجرا کنید تا زیرساخت تمامی نرم افزارها برقرار باشد و بعد از اجرای داکر کامپوز حتماٌ یک الی دو دقیقه صبر نمایید. زیرا که اپلیکیشن های common و parsa و restora در startup خود به این سرویس ها سعی می کنند وصل شوند و اگر ارتباط برقرار نشود به خطا خواهند خورد و نیاز به down کردن داکر کامپوز و up کردن مجدد آن خواهید داشت. نکته مهم دیگر این است که قبل از اجرای داکر کامپوز parsa حتماً gateway را اجرا کنید. زیرا در شروع نرم افزار بک اند پرسا همسان سازی شعبه ها نیز انجام میشود و آدرس مورد نظر پرسا بر روی gateway تعریف شده است و راه ارتباطی آن به واسطه gateway برقرار می شود.

## 🔸 دسترسی به دیتای داخل دیتابیس‌ها

چنانچه نیاز به دسترسی به دیتای داخل دیتابیس‌های mssql داشتید توصیه میشود از نرم افزار `Azure Data Studio` استفاده نمایید. این نرم افزار بسیار سبک است و برای تمامی سیستم عامل ها وجود می باشد. البته اگر توسعه دهندگانی بر روی سیستم خود `Mssql Management Studion` دارند نیاز به نصب اپلیکیشن دیگری ندارند و امکانات کاملتری را در اختیار دارند.
